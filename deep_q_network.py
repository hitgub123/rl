...
class DeepQNetwork:
    ...
    OBSERVE = 2.  # timesteps to observe before training
    ...
    def _build_net(self,input_shape):
        ...
        # pool2 = keras.layers.MaxPooling2D(pool_size=(2, 2), padding='same', data_format='channels_last')
        # model.add(pool2)
    ...
    def choose_action(self,s_t):
        if self.step % self.FRAME_PER_ACTION:
            ...
            if np.random.uniform()>self.epsilon:
                ...
                # print('\t{} <<<choose_action {}'.format(action,actions_value))
            else:
                action = np.random.randint(0,self.n_actions)
                # print('\t{} <<<random choose_action'.format(action))
        return tf.one_hot(action,self.n_actions)

    #分开获取记忆，合并后打乱
    def choose_batch_data(self):
        sample_index = np.random.choice(min(self.memory_count, self.memory_size), self.batch_size-self.batch_plus_size-self.batch_minus_size)
        sample_plus_index = np.random.choice(min(self.memory_plus_count, self.memory_plus_size), self.batch_plus_size)
        sample_minus_index = np.random.choice(min(self.memory_minus_count, self.memory_minus_size), self.batch_minus_size)
        x1,x2,x3=self.memory[sample_index], self.memory_plus[sample_plus_index], self.memory_minus[sample_minus_index]
    ...
    def learn(self):
        ...
        q_next=self.model.predict(o_s_ ,batch_size=self.batch_size)
        q_eval = self.model.predict(o_s,batch_size=self.batch_size)

        q_target = q_eval.copy()
        target_part=r_s + (1-t_s) * self.gamma * np.max(q_next,axis=1)
