...
class Prioritized_Replay_DQN:
    ...
    placeholder_weights = np.zeros((batch_size,))
    placeholder_y_true = np.zeros((batch_size, 2))
    ...
    def __init__(self, is_train=True):
        ...
        if is_train:
            self.FRAME_PER_ACTION = 5
            self.INITIAL_EPSILON = 1
        else:
            self.FRAME_PER_ACTION = 1
            self.INITIAL_EPSILON = 0.0001
        print(self.FRAME_PER_ACTION, self.INITIAL_EPSILON)
    ...
    def _build_net(self, input_shape, weights_shape=(1,), y_true_shape=(n_actions,)):
        input1 = tf.keras.layers.Input(shape=input_shape)
        weights = tf.keras.layers.Input(shape=weights_shape)
        y_true = tf.keras.layers.Input(shape=y_true_shape)
        # 卷积1
        ...
        # fc2
        output1 = keras.layers.Dense(self.n_actions, kernel_initializer=self.kernel_initializer)(output1)

        model = keras.models.Model(inputs=(input1, weights, y_true), outputs=output1)

        optimizer = keras.optimizers.Adam(learning_rate=self.lr)
        # optimizer = keras.optimizers.RMSprop()
        model.add_loss(self.mse_with_weight(output1, y_true, weights))
        model.compile(loss=None, optimizer=optimizer, metrics=['mse'])

        return model

    def mse_with_weight(self, y_pred, y_true, weights):
        # weights = tf.reshape(weights_, (-1, 1))
    ...
    def choose_action(self, s_t):
        if self.step % self.FRAME_PER_ACTION:
            ...
            if np.random.uniform() > self.epsilon:
                s_t = s_t[np.newaxis, :]
                actions_value = self.model.predict((s_t, np.zeros((1,)), np.zeros((1, 2))))
    ...
    def learn(self):
        ...
        # 改用ddqn
        q_eval = self.model.predict((o_s, self.placeholder_weights, self.placeholder_y_true),
                                    batch_size=self.batch_size)
        q_next = self.model.predict((o_s_, self.placeholder_weights, self.placeholder_y_true),
                                    batch_size=self.batch_size)
        q_next_target = self.model_target.predict((o_s_, self.placeholder_weights, self.placeholder_y_true),
                                                  batch_size=self.batch_size)

        q_target = q_eval.copy()
        max_next_action = np.argmax(q_next, axis=1)

        target_part = r_s + self.gamma * q_next_target[range(self.batch_size), max_next_action]
        ...
        history = self.model.train_on_batch((o_s, ISWeights, q_target))
    ...
    def train(self):
        ...
        while 1:
            ...
            if (self.step >= self.memory_size) and (self.step % self.learn_step == 0) and self.is_train:
...
dqn = Prioritized_Replay_DQN(is_train=1)
dqn.train()
